...
ScreenLine = int
ScreenColumn = int
SelectionInLine = Union[Tuple[ScreenColumn, ScreenColumn], Tuple[None, None]]


PositionBase = NamedTuple(
    "Position", [("x", ScreenColumn), ("y", ScreenLine), ("top_line", AbsoluteLine)]
)
...
class Position(PositionBase):
    ...
    def moved(self, dx: int = 0, dy: int = 0, dtop: int = 0) -> "Position":
        ...
        return self._replace(
            x=self.x + dx, y=self.y + dy, top_line=self.top_line + dtop
        )
    ...
    def scrolled_up(self, rows: ScreenLine) -> "Position":
        ...
        """
        return self.scrolled(-min(self.top_line - 1, rows - 1 - self.y))

    def scrolled_down(self, rows: ScreenLine, lines: AbsoluteLine) -> "Position":
        """
        Return a new position equivalent to self
        but with top_line as large as possible.
        """
        return self.scrolled(min(lines - rows + 1 - self.top_line, self.y))

    def scrolled_towards(
        self, other: "Position", rows: ScreenLine, lines: Optional[AbsoluteLine] = None
    ) -> "Position":
        """
        Return a new position equivalent to self.
...
class Region:
    ...
    def selection_in_line(
        current_line: int, start: Position, end: Position, maxx: int
    ) -> SelectionInLine:
        """
    ...
    @staticmethod
    def lines_affected(
        mark: Optional[Position], old_point: Position, point: Position
    ) -> Set[AbsoluteLine]:
        """
    ...
    @staticmethod
    def page_up(
        mark: Optional[Position], point: Position, rows: ScreenLine, lines: AbsoluteLine
    ) -> Position:
        """
        ...
        assert point.y == 0
        return Position(point.x, 0, max(1, point.top_line - rows + 1))
    ...
    def page_down(
        ...
        assert point.y == maxy
        return Position(point.x, maxy, min(lines - maxy, point.top_line + maxy))
...
class MarkedRegion(Region):
    ...
    def page_up(
        ...
        return Region.page_up(mark, point, rows, lines).scrolled_towards(
            mark, rows, lines
        )
    ...
    def page_down(
        ...
        assert mark is not None
        return Region.page_down(mark, point, rows, lines).scrolled_towards(
            mark, rows, lines
        )
...
class StreamRegion(MarkedRegion):
    ...
    @staticmethod
    def selection_in_line(
        current_line: AbsoluteLine, start: Position, end: Position, maxx: ScreenColumn
    ) -> SelectionInLine:
        if StreamRegion.line_outside_region(current_line, start, end):
            return None, None
        return (
            start.x if current_line == start.line else 0,
...
class ColumnarRegion(MarkedRegion):
    ...
    def adjust(start: Position, end: Position) -> Tuple[Position, Position]:
        return (
            start._replace(x=min(start.x, end.x)),
...
def load_config(*paths: str, overrides: Optional[Iterable[str]] = None) -> Options:
    ...
    opts_dict, paths = _load_config(
        defaults, parse_config, merge_result_dicts, *configs, overrides=overrides
    )
...
def string_slice(
    s: str, start_x: ScreenColumn, end_x: ScreenColumn
) -> Tuple[str, bool]:
...
class GrabHandler(Handler):
    def __init__(self, args: Namespace, opts: Options, lines: List[str]) -> None:
        super().__init__()
        self.args = args
        self.opts = opts
        self.lines = lines
    ...
    def _update(self) -> None:
        self.cmd.set_window_title(
            "Grab – {} {} {},{}+{} to {},{}+{}".format(
                self.args.title,
                self.mark_type.name,
                getattr(self.mark, "x", None),
                getattr(self.mark, "y", None),
    ...
    def _redraw(self) -> None:
        self._redraw_lines(
            range(self.point.top_line, self.point.top_line + self.screen_size.rows)
        )
    ...
    def on_key_event(
        self, key_event: KeyEvent, in_bracketed_paste: bool = False
    ) -> None:
        action = self.shortcut_action(key_event)
        if key_event.type not in [kk.PRESS, kk.REPEAT] or action is None:
            return
    ...
    def up(self) -> Position:
        return (
            self.point.moved(dy=-1)
            if self.point.y > 0
            else self.point.moved(dtop=-1) if self.point.top_line > 0 else self.point
        )
    ...
    def last_nonwhite(self) -> Position:
        ...
        return Position(
            wcswidth(line[: len(line) - len(suffix)]), self.point.y, self.point.top_line
        )

    def last(self) -> Position:
        return Position(self.screen_size.cols, self.point.y, self.point.top_line)
    ...
    def bottom(self) -> Position:
        ...
        y = min(len(self.lines) - self.point.top_line, self.screen_size.rows - 1)
    ...
    def _select_by_word_characters(self) -> str:
        return self.opts.select_by_word_characters or (
            json.loads(os.getenv("KITTY_COMMON_OPTS", "{}")).get(
    ...
    def _is_word_char(self, c: str) -> bool:
        return (
            unicodedata.category(c)[0] in "LN" or c in self._select_by_word_characters
        )

    def _is_word_separator(self, c: str) -> bool:
        return (
            unicodedata.category(c)[0] not in "LN"
    ...
    def _select(self, direction: DirectionStr, mark_type: Type[Region]) -> None:
    ...
    def select(self, region_type: RegionTypeStr, direction: DirectionStr) -> None:
        ...
        self.result = {
            "copy": "\n".join(
                ...
                for start_x, end_x in [
                    self.mark_type.selection_in_line(line, start, end, len(plain))
                ]
...
(Internal)"""
    ...
    try:
        ...
        tty = open(os.ctermid())
        lines = (
            sys.stdin.buffer.read().decode("utf-8").split("\n")[:-1]
        )  # last line ends with \n, too
        sys.stdin = tty
        opts = load_config()
